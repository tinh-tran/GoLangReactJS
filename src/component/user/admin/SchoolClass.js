...
import { Table, Button, Alert, message, Divider, Modal } from 'antd'
...
class SchoolClass extends React.Component {
  ...
  online(classid) {
    axios.post('/api/class/changedisplay', { classid: classid, sqltable: 'schoolclass', display: '是' }).then(res => {
      message.success('上线成功')
      this.fecthdata()
    })
  }
  outline(classid) {
    axios.post('/api/class/changedisplay', { classid: classid, sqltable: 'schoolclass', display: '否' }).then(res => {
      message.success('下线成功')
      this.fecthdata()
    })
  }
  fecthdata() {
    axios.post('/api/class/school/get').then(res => {
      this.setState({
  ...
  render() {
    return (
      <div>
        ...
        >
          <SelectClass sqltable="schoolclass" name="学校专区" />
        </Modal>
      </div>
    )
  }
