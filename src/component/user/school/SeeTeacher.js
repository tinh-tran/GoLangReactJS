...
class SeeTeacher extends React.Component {
    state = {
        peopleList: [],
        message: "当前一共有0个教师！"
    };
    componentWillMount() {
        axios.post('/api/school/findpeople', { type: '教师', belong: this.props.user.belong }).then(res => {
            res.data.map((v, index) => {
                v.key = index + 1;
            })
            this.setState({
                peopleList: res.data,
                message: `当前一共有${res.data.length}个教师！`
            })
        })
    }
    render() {
